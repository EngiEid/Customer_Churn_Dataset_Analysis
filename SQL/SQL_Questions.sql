...
-- 10.What is the average number of support calls for churned customers?
select AVG([Support Calls]) as Avg_Support_Calls
from Fact_Customer_Churn
where Churn=1

-- 11. Is a higher number of support calls associated with increased churn?
SELECT 
    [Support Calls],
    AVG(CAST(Churn AS DECIMAL(10,4))) * 100 AS ChurnRate
FROM Fact_Customer_Churn 
...
SELECT
    Churn,
    CAST(AVG([Total Spend]) AS INT) AS AvgPaymentDelayDays
FROM Fact_Customer_Churn
GROUP BY Churn
ORDER BY Churn
-- 15. Do low-spending customers have higher churn?
SELECT Top 10
    f.[Payment Delay],
    AVG(CAST(f.Churn AS DECIMAL(10,4))) * 100 AS ChurnRate
FROM Fact_Customer_Churn f JOIN Dim_Customer d ON d.CustomerID = f.CustomerID
...
-- 16. Which subscription types have the highest churn rate?
SELECT top 1
    d.[Subscription Type],
    AVG(CAST(f.Churn AS DECIMAL(10,4))) * 100 AS ChurnRate
...
--17. How does subscription types affect churn?
SELECT 
    d.[Subscription Type],
    AVG(CAST(f.Churn AS DECIMAL(10,4))) * 100 AS ChurnRate
FROM Fact_Customer_Churn f JOIN Dim_Subscription d  
